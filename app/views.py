...
class FoodPlaceListCreateView(generics.ListCreateAPIView):
    """
    GET: List địa điểm ăn uống (filter ?destination_id=)
    POST: Chỉ admin được phép tạo mới
    """
    ...
    def get_queryset(self):
        qs = FoodPlace.objects.select_related("destination").all()
        destination_id = self.request.query_params.get("destination_id")
        if destination_id:
            qs = qs.filter(destination_id=destination_id)
        return qs
...
class FoodPlaceDetailView(generics.RetrieveUpdateDestroyAPIView):
    """
    GET: Xem chi tiết 1 địa điểm ăn uống
    PUT/PATCH/DELETE: Chỉ admin
    """
    queryset = FoodPlace.objects.select_related("destination").all()
    serializer_class = FoodPlaceSerializer
    permission_classes = [permissions.IsAuthenticatedOrReadOnly]
